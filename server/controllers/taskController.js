...
const dashboardStatistics = asyncHandler(async (req, res) => {
  try {
    ...
    const previousMonthTasks = allTasks.filter(
      (task) =>
        task.createdAt >= previousMonthStart &&
        task.createdAt < currentMonthStart
    );
    const lastMonthTasks = previousMonthTasks.reduce(
      (result, task) => {
        result.total += 1;
        result[task.stage] += 1;
        return result;
